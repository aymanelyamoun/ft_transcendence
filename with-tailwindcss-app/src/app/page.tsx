 ...
 const gradientStyle = {
    ...
    return (
      ...
      <div style={gradientStyle} className='fixed top-8 left-1/4 transform -translate-x-1/2 -rotate-6 w-[200px] h-[423px] rounded-lg'></div>
        <div style={{ background: '#9A9BD3'  } } className='fixed top-8 right-1/4 transform translate-x-1/2 rotate-6 w-[30px] h-[323px] rounded-lg '></div>
      <h2  className=" text-white shadow-2xl  text-7xl font-bold mb-3"> PONG</h2>
      <div className=' py-20'>
        <a style={gradientStyle} href="/signup" className=' m = 0 text-white rounded-full px-24 py-2 inline-block font-semibold hover:bg-sky-950 mb-7 shadow-xl'>PLAY</a>
      </div>
    </div>
  );
}
